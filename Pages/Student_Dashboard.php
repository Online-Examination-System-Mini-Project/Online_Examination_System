    ...
    <div id="container">
        ...
        if(@$_GET['q']==3){
            ...
            if($rowcount == 0){
                ...
                <td onclick=myfunction("'. $eid. '","'. $total. '","'. $time. '") class="pull-right btn sub1" style="margin:0px;background:#99cc32"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Start</b></span></td></tr>';
                echo '<script>function myfunction(){
                    window.open("exam.php?username='.$user_name.'&email='.$email.'&q=quiz&step=2&eid=".concat(arguments[0],"&n=1&t=",arguments[1],"&time=",arguments[2]),"", "width=3000,height=3000");
                }</script>';
            }
            else{
                echo '<tr><td>'.$c++.'</td><td>'.$title.'</td><td>'.$total.'</td><td>'.$sahi*$total.'</td><td>'.$time.'&nbsp;sec</td>
